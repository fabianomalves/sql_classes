...
SELECT
    ...
    ROUND(
        COUNT(*) / CAST(
            COUNT(DISTINCT t1.order_id) AS FLOAT
        ),
        2
    ) AS average_items_by_orders
FROM
    tb_order_items AS t1
LEFT JOIN tb_products AS t2 
ON
...
/* What sellers didn't sale in December 2017? 
 * Filter by state
 *  Consider the whole base, only with delivered requests. 
 */
SELECT
    t2.seller_id,
    t3.seller_state,
    --The max return the maximum transation between 0 and 1
    --and return the 1
    MAX(CASE
